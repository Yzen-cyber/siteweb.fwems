...
class Logger {
    ...
    static clearOldLogs(hours = 24) {
        const cutoffTime = new Date(Date.now() - (hours * 60 * 60 * 1000));
        logsData = logsData.filter(log => new Date(log.timestamp) > cutoffTime);
        localStorage.setItem('emsLogs', JSON.stringify(logsData));
...
function loadDashboard() {
    ...
    if (window.staffData && window.salaryData) {
        ...
        window.staffData.forEach(staff => {
            ...
            if (salaryInfo && salaryInfo.total) {
                // Convertir en salaire mensuel (semaine * 4.3)
                const monthlySalary = salaryInfo.total * 4.3;
...
function clearOldLogs() {
    ...
    const remainingCount = Logger.clearOldLogs(24);
    const deletedCount = logsBefore - remainingCount;

    showNotification(`${deletedCount} enregistrement(s) supprimé(s)`, 'success');
    loadLogs();
}
...
function initializeApp() {
    ...
    // Charger et appliquer les paramètres utilisateur
    ...
    if (settings.animationsEnabled !== undefined) applyAnimations(settings.animationsEnabled);
    if (settings.sidebarCollapsed) applySidebar(settings.sidebarCollapsed);
...
function loadSettingsModal() {
    ...
    document.getElementById('extendedSession').checked = settings.extendedSession;
    ...
    document.getElementById('activityTracking').checked = settings.activityTracking;
    document.getElementById('animationsEnabled').checked = settings.animationsEnabled;
    document.getElementById('sidebarCollapsed').checked = settings.sidebarCollapsed;
...
function saveSettings() {
    const settings = {
        ...
        extendedSession: document.getElementById('extendedSession').checked,
    ...
    applySidebar(settings.sidebarCollapsed);
    applyFullscreen(settings.fullscreenMode);
...
function applyTheme(theme) {
    ...
    // Appliquer les couleurs spécifiques selon le thème
...
function applyDensity(density) {
    ...
    switch(density) {
        case 'compact':
            ...
            break;
        case 'spacious':
            root.style.setProperty('--spacing-small', '16px');
            root.style.setProperty('--spacing-medium', '32px');
            root.style.setProperty('--spacing-large', '48px');
            break;
        default: // comfortable
            root.style.setProperty('--spacing-small', '12px');
            root.style.setProperty('--spacing-medium', '24px');
            root.style.setProperty('--spacing-large', '36px');
            break;
    }
}
...
function exportSettings() {
    ...
    link.href = url;
    link.download = `settings_${currentUser.email}_${new Date().getTime()}.json`;
    link.click();
    
    Logger.log('SETTINGS_EXPORT', `Export des paramètres pour ${currentUser.email}`);
...
// GESTION CODES RADIO
// ==============================
function initCodesRadio() {
    Logger.log('CODES_INIT', 'Initialisation des codes radio');
    // Les codes sont déjà dans le HTML, pas besoin d'initialisation supplémentaire
}

function filterCodes() {
    const searchTerm = document.getElementById('codeSearch').value.toLowerCase();
    const categoryFilter = document.getElementById('codeCategoryFilter').value;
    const codeCards = document.querySelectorAll('.code-card');
    
    codeCards.forEach(card => {
        const codeNumber = card.querySelector('.code-number').textContent.toLowerCase();
        const description = card.querySelector('.code-description').textContent.toLowerCase();
        const response = card.querySelector('.code-response').textContent.toLowerCase();
        const category = card.getAttribute('data-category');
        
        const matchesSearch = codeNumber.includes(searchTerm) || 
